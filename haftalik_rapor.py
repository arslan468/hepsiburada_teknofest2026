    ...
    for cikis in sehirler:
        for varis in sehirler:
            ...
            desi_listesi.append({
                ...
                'Çıkış Şehri': cikis,
                'Varış Şehri': varis,
                'Tahmini Desi': round(hedef_desi, 2)
            })
            ...
            atamalar = []
            if kendi_tir_adet > 0: atamalar.append(("Kiralık Tır", kendi_tir_adet * fiyatlar["kiralik_tir_fiyat"]))
            if kendi_kamyon_adet > 0: atamalar.append(("Kiralık Kamyon", kendi_kamyon_adet * fiyatlar["kiralik_kamyon_fiyat"]))
            if spot_tir_adet > 0: atamalar.append(("Spot Tır", spot_tir_adet * fiyatlar["spot_tir_fiyat"]))
            if spot_kamyon_adet > 0: atamalar.append(("Spot Kamyon", spot_kamyon_adet * fiyatlar["spot_kamyon_fiyat"]))
            
            if not atamalar:
                atama_listesi.append({
                    'Tarih': tarih_str,
                    'Araç Tipi': 'Atama Yapılamadı',
                    'Çıkış Şehri': cikis,
                    'Varış Şehri': varis,
                    'Tahmini Desi': round(hedef_desi, 2),
                    'Maliyet (TL)': 0
                ...
                for arac_turu, maliyet in atamalar:
                    atama_listesi.append({
                        'Tarih': tarih_str,
                        'Araç Tipi': arac_turu,
                        'Çıkış Şehri': cikis,
