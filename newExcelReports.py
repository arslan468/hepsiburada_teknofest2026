...
if __name__ == "__main__":
    ...
    df_ana = pd.DataFrame(rapor_satirlari)
    
    df_talep = df_ana[['Tarih', 'Çıkış Merkezi', 'Varış Merkezi', 'Mesafe (KM)', 'YZ Tahmini Yük (Desi)']]
    talep_dosyasi = 'Tahmini_Talep.xlsx'
    df_talep.to_excel(talep_dosyasi, index=False)
    
    df_arac = df_ana[['Tarih', 'Çıkış Merkezi', 'Varış Merkezi', 'Kendi Tırımız', 'Kendi Kamyonumuz', 'Spot Tır (Kiralık)', 'Spot Kamyon (Kiralık)', 'Optimum Maliyet (TL)']]
    arac_dosyasi = 'Arac_Tahmin.xlsx'
    df_arac.to_excel(arac_dosyasi, index=False)
    
    print("\n" + "="*50)
    print("🚀 İŞLEM TAMAM! İki ayrı dosya başarıyla oluşturuldu:")
    print(f"1) {talep_dosyasi} -> (Yapay zekanın kargo yükü tahminleri)")
    print(f"2) {arac_dosyasi} -> (Matematik motorunun araç atamaları ve faturası)")
    print("="*50)